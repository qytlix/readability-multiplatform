...
describe('EntryQueryCompiler', () => {
  ...
  it('rejects malformed queries before any database access', () => {
    ...
    } as unknown as EntryQuery)).toThrow('Entry query filter field is invalid.');
    expect(() => compileEntryQueryScope({
      limit: 20,
  ...
  it('keeps exact tag placeholders and boolean negation in compiled SQL', () => {
    const result = compileEntryQueryScope({
      limit: 20,
      filters: [
    ...
    expect(result.conditions[0]).toContain('t.name = ?');
    expect(result.conditions[1]).toBe('e.isRead != ?');
    expect(result.parameters).toEqual(['AI', 1]);
  });
});
