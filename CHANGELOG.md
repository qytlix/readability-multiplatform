...
- 深度翻译加入专用提示、局部上下文、结果变体与分阶段检查点，并在 Reader
  中提供相应的进度和状态提示。（PR #125）
- Article Chat 支持围绕当前文章连续对话，可添加文本、PDF 与图片附件、
  针对 Reader 选区提问、恢复持久化会话，并为对话单独选择受支持的
  Provider 与模型。（PR #132）

### Changed

- 收敛 Article Chat 的失败诊断、结构化日志与 Usage 记录边界，使每次实际
  Provider 请求拥有明确且可追踪的用量归属。（`7a9fddd94a9b`）

### Fixed

- 修复标签精确匹配查询缺少 SQL 参数占位符的问题，恢复侧边栏标签的精确
  包含与排除筛选。（PR #113）
- Reader 图片统一使用 `no-referrer` 策略，避免部分跨域图片因 ORB/403
  被阻止加载。（PR #136）
- 清理正文中混入的作者资料卡、评论容器与 Follow CTA，减少阅读、翻译和
  导出内容中的页面噪声。（PR #131）
- 对话在上下文准备阶段即可取消，并增加竞态隔离；Entry List 读取失败改为
  安全、稳定的错误展示，不再向界面暴露数据库细节。（PR #142）

### Docs

- 更新项目介绍、功能与平台状态、开发和验证说明，并补充界面截图。
  （PR #142、`bf6468c6dc24`、`d764cda9b7c3`）
