...
function appendSingleFilter(
  ...
  switch (field) {
    ...
    case 'feed': {
      conditions.push(
        `search_normalize(f.title) ${operator === '-' ? 'NOT ' : ''}LIKE ?${escapeClause}`,
      );
    ...
    case 'title': {
      conditions.push(
        `search_normalize(e.title) ${operator === '-' ? 'NOT ' : ''}LIKE ?${escapeClause}`,
      );
      parameters.push(`%${escapeLikePattern(value)}%`);
      break;
    }
...
function appendOrGroupFilter(
  ...
  switch (field) {
    ...
    case 'starred':
    case 'read': {
      const hasYes = values.some((value) => value === 'yes' || value === '1');
      const hasNo = values.some((value) => value === 'no' || value === '0');
      if (hasYes && hasNo) {
        conditions.push('1 = 0');
      } else {
        const column = field === 'starred' ? 'e.isStarred' : 'e.isRead';
        conditions.push(`${column} = ${hasYes ? '1' : '0'}`);
      }
      break;
    }
  }
