...
1. **提取分段**：将文章的 Markdown 渲染为 HTML，在 DOM 中按深度优先遍历
   - 只取 `<p>`、`<ul>`、`<ol>` 三种元素为可分段
...
翻译任务的输出结果。按 `(entryId, targetLanguage, sourceContentHash, segmenterVersion)` 唯一约束，确保源内容未变时不重复翻译。
...
| `entryId` | INTEGER | NOT NULL → entry(id) CASCADE | 目标文章 |
| `targetLanguage` | TEXT | NOT NULL | 目标语言 |
| `sourceContentHash` | TEXT | NOT NULL | 源内容哈希，用于检测内容变更 |
| `segmenterVersion` | TEXT | NOT NULL | 分词器版本 |
| `outputLanguage` | TEXT | NOT NULL | 实际输出语言 |
| `runStatus` | TEXT | NOT NULL | 运行状态：`running` / `succeeded` |
| `createdAt` | DATETIME | NOT NULL | 创建时间 |
| `updatedAt` | DATETIME | NOT NULL | 更新时间 |

**索引：**
- `idx_translation_slot` — `(entryId, targetLanguage, sourceContentHash, segmenterVersion)` 唯一
- `idx_translation_updated` — `updatedAt`

---

### translation_segment — 翻译分段

翻译结果的双语对照段落。一个翻译结果包含多个分段（`p`、`ul`、`ol`），按顺序排列。

| 列 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `taskRunId` | INTEGER | NOT NULL → translation_result(id) CASCADE | 关联翻译结果 |
| `sourceSegmentId` | TEXT | NOT NULL | 源段 ID |
| `orderIndex` | INTEGER | NOT NULL | 段落序号 |
| `sourceTextSnapshot` | TEXT | — | 源文本快照 |
| `translatedText` | TEXT | NOT NULL | 翻译文本 |
| `createdAt` | DATETIME | NOT NULL | 创建时间 |
| `updatedAt` | DATETIME | NOT NULL | 更新时间 |

**索引：**
- `idx_translation_segment_order` — `(taskRunId, orderIndex)`
- `idx_translation_segment_unique` — `(taskRunId, sourceSegmentId)` 唯一
