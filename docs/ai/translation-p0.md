...
Translation translates one persisted Reader article from an automatically detected or explicitly selected language into English, Simplified Chinese, Hong Kong Traditional Chinese, Japanese, Korean, German, French, or Spanish and presents a block-aligned bilingual projection in Reader mode. The projection keeps the sanitized Reader HTML as its layout skeleton, so figures, standalone images, tables, code blocks, and other non-translatable nodes remain in their original positions while translated blocks are inserted alongside their source blocks. It uses the active OpenAI, DeepSeek, OpenRouter, Anthropic, Gemini, or custom OpenAI-compatible profile through the same Main-only secret boundary as Summary.

The translation source is the sanitized `CleanedContent.cleanedHtml` contract plus Reader title/byline metadata. `ContentSegmenter` v4 emits deterministic semantic blocks for `title`, `byline`, `heading`, `paragraph`, `list`, `blockquote`, and `caption`. Each `<li>` is an independent `list` segment; direct `<p>` and `<cite>` children of a blockquote are independent `blockquote` segments. Nested list containers are excluded from their parent item snapshot so child items are not translated twice. Content from older segmenter versions is rebuilt lazily before Translation. Each segment has a stable ID, and the complete normalized segment sequence produces a SHA-256 `sourceContentHash`.
...
Structured logging records each Translation start plus aggregate completed/failed run summaries with provider-request, batch, compensation, omission, duration, and provider-reported token counts. `missingSegmentCount` is the number of Provider-omitted segments for which compensation was initiated; `unresolvedMissingSegmentCount` is the subset that did not finish successfully after compensation. Successful individual Provider requests do not create persistent records; interruption remains a run-level lifecycle record. Exceptional request failures and omissions retain the task run ID, opaque execution attempt ID, and Provider request ID for correlation across retries, while still never logging the prompt, article text, API key, Authorization header, or provider configuration.
...
13. Generate one quality sample for every target language. For `zh-HK`, verify Hong Kong terms and orthography are used rather than default Taiwan wording; then change only the source selector and confirm the previous cache is not silently reused.
